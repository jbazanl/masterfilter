...
def amarilla(request):
    if request.user.is_authenticated:
        ...
        if doc:
            total = doc.total
            name = str(doc.file)
        return render(request, "amarilla.html",{
                    "segment":"amarilla", 
                    "subido": total, 
                    "endpoint": endp, 
                    "perm": perm, 
...
def send_data(user, data, op, name_file):
    #print(data)
    url = "http://127.0.0.1:8008/process/"
    payload = json.dumps({
        "user": user,
        "number": data,
        "new":op,
